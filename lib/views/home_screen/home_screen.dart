...
import 'package:unibank/views/transactions/locker/saving_locker.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> schedulePeriodicExecution() async {
    ...
    // Get the last execution date from shared preferences
    String? lastExecutionDateString = prefs.getString('lastExecutionDate');
    DateTime lastExecutionDate = lastExecutionDateString != null
        ? DateTime.parse(lastExecutionDateString)
        : now;

    // Check if the last execution date is not the same as today's date
    if (lastExecutionDate.year != now.year + 1 ||
        lastExecutionDate.month != now.month ||
        lastExecutionDate.day != now.day) {
      // Call your function here
      await userController.checkAndPrintRecentSubscriptions();
      await userController.checkAndNotifyIncomingTransactions();
      // Update the last execution date to today
      await prefs.setString('lastExecutionDate', now.toString());
    }
  }
